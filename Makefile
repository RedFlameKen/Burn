...
build:
	...
	clang -x c -std=c11 -DDISPLAY_BACKEND=SDL_BACKEND -o bin/circle src/demo/circle.c -lm $(shell pkg-config --cflags --libs $(libs)) $(flags)
...
build-wasm:
	...
	clang -fno-builtin --target=wasm32 --no-standard-libraries -DDISPLAY_BACKEND=WASM_BACKEND -nostdlib -Wl,--no-entry -Wl,--export-all -Wl,--allow-undefined -o bin/circle.wasm src/demo/circle.c
